...
public final class CommentaryPromptBuilder {
    ...
    public static final String SYSTEM_PROMPT = """
            ...
            2) 라인별로 두 팀 선수를 짝지어 훑는다. 나머지 네 라인도 짧게라도 모두 언급해 10명 전원의 이름이 한 번 이상 나오게 한다
               ('팀 구성' 참고: 계급 분포, 칭호, 주 챔피언 조합을 인물 소개에 섞는다)
            ...
            - 참가한 10명 전원을 최소 한 번씩 이름으로 부른다. 주목받는 선수는 길게, 나머지는 반 문장이라도 짚는다. 빠진 선수가 있으면 실패다.
    ...
    static final List<String> LENGTHS = List.of(
            "표준: 5~6문장, 전체 450자 이내. 10명 전원 이름 언급",
            "표준: 5~6문장, 전체 450자 이내. 10명 전원 이름 언급",
            "라인 훑기: 라인별 한 문장씩 5문장 + 마무리 1문장, 전체 550자 이내. 10명 전원 이름 언급"
    );
    ...
    public static String userPrompt(String roomName, List<TeamSlot> slots, TeamBalance balance,
        ...
        sb.append("[/DATA]\n\n위 데이터로 경기 전 해설을 해줘. 참가한 10명 전원의 이름을 한 번씩은 꼭 불러줘.");
        return sb.toString();
    }
